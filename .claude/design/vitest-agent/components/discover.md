...
- **classify.** Filename-suffix match — files ending in .e2e.(test|spec).
  ...
  ["unit"].
- **buildProject.** Scans the package for test files with a single `findTestFiles` walk against two patterns — `src/**/*.{test,spec}.{ts,tsx,js,jsx}` and `__test__/**/*.{test,spec}.{ts,tsx,js,jsx}`. Returns null if neither pattern matches. Otherwise emits a `TestProjectInlineConfiguration` with `extends: true`, the test name set from the package name, `environment: "node"`, absolute include globs — `<path>/src/**/*.{test,spec}.{ts,tsx,js,jsx}` and/or `<path>/__test__/**/*.{test,spec}.{ts,tsx,js,jsx}` — anchored at the package root and covering whichever of the two buckets produced matches, an exclude list, and a `setupFiles` entry for `vitest.setup.(ts|tsx|js|jsx)` at the package root when present. Results are bucketed by "under `src/` or not": a test file is discoverable only at those two anchored locations, full stop. A `__test__/` directory nested anywhere else in the package tree — `lib/scripts/__test__/`, for example — is never included; the same `classifyTestPath` rule in `@vitest-agent/sdk`'s `utils/test-location.ts` that these globs are generated from also backs the `check-test-path` CLI subcommand and the PreToolUse hook, so there is exactly one implementation of the rule. That single implementation is the *default* strategy's rule and nothing more: a consumer who swaps in a custom `discoverStrategy` (or `discoverStrategy: false`), chains `.addProject(...)`, or subclasses `DefaultDiscoverStrategy` may collect paths the rule calls `invalid`, so `check-test-path` scans the workspace's Vitest/Vite config source for those markers with the SDK's pure `detectNonDefaultDiscoverStrategy` and fails open — no verdict, hook no-ops — when it finds one or cannot read the config at all (issue #230; [../decisions.md](../decisions.md) Decision 61). The scan is lexical by design: loading the consumer's config on the hook hot path is out of scope, and the check's honest contract is "the default layout". (Issue #227: an unanchored `**/__test__/**` glob, added to reach the path reported in issue #184, escaped the package boundary — Vitest globs literally and has no nested-`package.json` concept, and for the root workspace, which `@effected/workspaces` reports at the repo root, that meant globbing the entire repository.) The exclude list is emitted unconditionally and is `configDefaults.exclude` (`**/node_modules/**`, `**/.git/**`) plus a bounded glob for every `NON_DISCOVERABLE_DIRS` entry under BOTH include roots (`<path>/{src,__test__}/**/{node_modules,.git,dist}/**`), plus — only when the `__test__` bucket produced matches — the helper-subdirectory globs (`utils`, `fixtures`, `snapshots`) anchored at the test root as `<path>/__test__/{fixtures,snapshots,utils}/**`. The helper globs are anchored the same way `include` is: only a helper directory *directly under* `__test__/` is excluded. The earlier `<path>/__test__/**/{fixtures,snapshots,utils}/**` form matched a helper-named directory at any depth, so an ordinary suite laid out as `__test__/unit/utils/foo.test.ts` — the natural mirror of `src/utils/` — was swept out of discovery with no warning (issue #251: a real consumer silently lost 5 suites / 60 cases). The same anchoring change was made in `classifyTestPath`, which now inspects only the segment directly under `__test__/`. The prepend is load-bearing: a custom `test.exclude` replaces Vitest's defaults rather than merging. The bounded `NON_DISCOVERABLE_DIRS` globs replace the former unanchored `**/dist/**`: an anchored include cannot reach a package's own top-level `dist/`, but it can reach one nested deeper (`src/foo/dist/x.test.ts`), and Vitest's defaults cover only `node_modules` and `.git`. Bounding each under both roots makes the emitted config prune exactly what `findTestFiles` prunes — the last divergence between the walker and the glob.
...
5. **Iterate packages.** For each package, call strategy.buildProject
   ...
   "discoverProjects" section of [./plugin.md](./plugin.md). (That
   command declines to answer for a workspace whose config marks a
   non-default strategy — see the `buildProject` description in the
   default-strategy section above.)
