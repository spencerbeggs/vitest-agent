...
**Per-project timeouts (issue #242).** `ProjectSummary` (in `packages/sdk/src/contracts/dispatcher.ts`) carries an optional `timeoutCount` (absent = 0), so the workspace-level projects table can now attribute timeouts to a project. `formatProjectRow` and `formatWorkspaceTotal` in `dispatcher/helpers.ts` fold it into the denominator, render an `N timed out` part after `N failed`, and pick the `✗` glyph when *either* failures or timeouts are nonzero — a project whose only non-passing test timed out is no longer painted `✓`. Both producers keep the reducer's convention that a timed-out test lives in `timeoutCount` and not `failCount`: `summarizeProject` in `@vitest-agent/reporter` derives it from the report (see [./reporter.md](./reporter.md)), and `StreamApp`'s `buildProjectSummary` takes it from the reducer counts. This closed the gap #224 left where the totals line and outcome class were correct but the projects table could not show which project timed out.
...
`packages/ui/src/dispatcher/helpers.ts` holds the shared formatters every cell uses: totals header, failure block, coverage judgment line, trend line, the projects table with name padding and tag-count suffix, the workspace total footer and the below-target table. The project row and workspace total both read `project.timeoutCount ?? 0`: the value joins the denominator, emits an `N timed out` part when nonzero, and (for the row) flips the glyph to `✗` alongside `failCount > 0` — see *Per-project timeouts (issue #242)* above. `packages/ui/src/dispatcher/ink-helpers.tsx` exports `renderAgentStringAsInk`, which wraps an agent string in colored Ink `<Text>` rows so cells can share their agent-half output as a default Ink render.
