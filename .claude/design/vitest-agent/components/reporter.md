...
**Partial runs reach both render paths the same way (issue #160).** The `agent` fold's `synthesizeFromAgentReport` copies `CoverageReport.scoped` / `scopedFiles` / `totalFiles` onto `CoverageReady`; the `stream` mount receives the same triple on the plugin's live emit. The reducer lands both on `RenderState.coverage`, and `dispatch` / `dispatchInk` append the `Coverage thresholds skipped: partial run (N of M test files)` note after the cell output — so this package needs no partial-run logic of its own. See [./ui.md](./ui.md) *Dispatch entry points*.
