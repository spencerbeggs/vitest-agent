...
The build mode is selected by the `SIDECAR_DIST_MODE` env var, which the npm `build:dev` / `build:prod` scripts set — tsdown has no `--env-mode` equivalent. `build:dev` (`SIDECAR_DIST_MODE=dev`) emits `dist/dev`; `build:prod` (`SIDECAR_DIST_MODE=prod`) emits `dist/github` and `dist/npm`. The three variants mirror the rslib packages' layout exactly: `dist/dev` is the workspace link target, `dist/github` the GitHub Packages publish target, `dist/npm` the npm registry target.
