...
`vitest-agent-sidecar` is a `peerDependency` of `vitest-agent-plugin`, declared alongside `vitest-agent-cli` and `vitest-agent-mcp`. For the package's build, distribution and the `inject-env`-only scope, see [./sidecar.md](./sidecar.md).
