...
### Decision 54: Caller-Declared, Validated `projectRoot` over Server-Side Worktree Detection

**Context.** The MCP server freezes its Vitest root at boot (`ctx.cwd`) and one long-lived process serves every caller. From a git worktree that produced a false green: `run_tests` ran the *other* tree and reported those results as the caller's, with nothing in the response naming which tree answered (issue #252). The obvious fix — have the server detect the mismatch and refuse — is not available: the server cannot observe the caller's cwd, so it has nothing to compare against.

**Decision.** Ship the half that works with no cooperating client, and ship it in two pieces. First, an optional `projectRoot` input that overrides `ctx.cwd` for one call. It is validated rather than trusted: the path must be an existing directory that shares a git common directory with `ctx.cwd`, checked with `git rev-parse --git-common-dir` — the one identifier that is identical across a repository and every worktree attached to it, where `--show-toplevel` differs per worktree and would reject exactly the sibling-worktree case the param exists for. Both candidates go through `realpath`, because git prints a relative `.git` from a main worktree and an absolute symlink-resolved path from a linked one. A path in a different repository, or one that does not exist, returns the tool's error envelope naming both paths; `createVitest` is never reached and there is no silent fallback to `ctx.cwd`. Second, and more important, `RunTestsOk` and `RunTestsNoMatch` both carry a **required** `projectRoot`, always populated, rendered as a `Project root:` line in the markdown. An agent that never learns the new param still sees which tree answered.
