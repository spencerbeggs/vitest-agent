/**
 * tsdown SEA build config for `vitest-agent-sidecar-linux-arm64`.
 *
 * This child package compiles ITS OWN platform binary — a Node Single
 * Executable Application (SEA) for `linux/arm64` only. The five
...
// `linux`; the published `os` value is `linux`.
const TARGET = { platform: "linux", arch: "arm64", nodeVersion: "25.9.0" } as const;
