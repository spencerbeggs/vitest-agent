...
/**
 * The server layer: every tool registered under the strict contract over
 * `McpServer.layerStdio`. `protocols` is newest-first because the registry
...
> =>
	Layer.mergeAll(registerStrictToolkit(Kit).pipe(Layer.provide(ToolsLayer))).pipe(
		Layer.provide(
