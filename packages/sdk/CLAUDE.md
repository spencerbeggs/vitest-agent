...
the repo-root `schemas/5.0/run.json` GitHub serves raw; generated by
`@effected/schemastore-cli` from `lib/configs/schemastore.config.ts`). Everything that touches a
...
lib/configs/        -- schemastore.config.ts + run-report-schema.ts: the
                         HostedSchema identity and `schemastore` config that
                         emit the repo-root `schemas/5.0/run.json`
                         (`pnpm --filter @vitest-agent/sdk schema:check`)
```
