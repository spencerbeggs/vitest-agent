...
| `utils/test-location.ts` | The single source of truth for the test-layout rule. Exports `SRC_DIR`, `TEST_DIR`, `TEST_HELPER_DIRS`, `TEST_FILE_GLOB_SUFFIX` constants and `classifyTestPath(workspaces, filePath)`, which classifies an absolute path as `valid` (under a workspace's `src/` or non-helper `__test__/`, anchored at the package root), `excluded` (under a `__test__/` helper dir — `fixtures`/`snapshots`/`utils`), or `invalid` (anywhere else, with a `suggestedPath`); returns `null` when no supplied workspace contains the path, meaning callers must fail open rather than treat it as a verdict. Consumed by `@vitest-agent/plugin`'s discovery include/exclude globs and cache-signature walk, and by `@vitest-agent/cli`'s `agent check-test-path` subcommand backing the `test-location.sh` PreToolUse hook |
