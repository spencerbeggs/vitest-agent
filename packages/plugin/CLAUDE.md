...
**The carrier (rank 5, issue #412).** This is the one package a consumer installs. It declares `@vitest-agent/cli`, `@vitest-agent/mcp`, `@vitest-agent/reporter`, `@vitest-agent/engine`, and `@vitest-agent/sdk` as regular `workspace:*` dependencies (published exact-pinned; a cli/mcp release patch-bumps the plugin) AND declares both bins itself — `bin.vitest-agent` → `src/bin/vitest-agent.ts` (`main({ distribution })` from `@vitest-agent/cli/main`) and `bin.vitest-agent-mcp` → `src/bin/vitest-agent-mcp.ts` (`void main({ distribution })` from `@vitest-agent/mcp/main`), `distribution` being `{ name: "@vitest-agent/plugin", version: CURRENT_PLUGIN_VERSION }`. pnpm links only direct-dependency bins, so these shims are what put `node_modules/.bin/vitest-agent` and `.bin/vitest-agent-mcp` into a consumer under every package manager; no `publicHoistPattern`, pnpm plugin, or manual step is involved. The Vitest-side peers (`vitest`, `@vitest/coverage-v8`, `@vitest/coverage-istanbul`) stay `peerDependencies`. The plugin does not depend on `@vitest-agent/ui`, `react`, or `ink` (reporter pulls them) nor on `@vitest-agent/sidecar` (cli pulls it). `ReporterLive(options: PlatformOptions)` is `CoverageAnalyzerLive` over the engine's `PlatformLive`.
...
src/
  ...
  bin/
    vitest-agent.ts     -- carrier shim over @vitest-agent/cli/main (passes distribution)
    vitest-agent-mcp.ts -- carrier shim over @vitest-agent/mcp/main (passes distribution)
  version.ts          -- CURRENT_PLUGIN_VERSION (read by the shims)
  ...
  utils/
    ...
    is-partial-run.ts          -- isPartialRun: pure scoped-run decision
    ...
    find-test-files.ts         -- async glob walker over the injected
                                   WalkerFileSystem with an inline glob-to-regex
                                   compiler; skips node_modules, .git, dist by
                                   default and stops at any nested package.json
                                   boundary
    walker-fs.ts               -- WalkerFileSystem / WalkerEntry / WalkerEntryStat
                                   port + the nodeWalkerFs binding every walker
                                   defaults to
...
| `reporter.ts` | Internal `AgentReporter` class. Imports `DefaultVitestAgentReporter` from `@vitest-agent/reporter`; imports nothing from `@vitest-agent/ui`. Creates an Effect `PubSub` run-event channel and publishes one `RunEvent` per Vitest streaming callback onto it; it wires every Vitest reporter hook, so the channel carries the complete `RunEvent` surface. The channel rides `ReporterKit.runEvents` (optional field). The reporter factory is invoked at run start (`onInit`) so a live reporter can subscribe before the first event — the plugin no longer drives the Ink mount, holds a `liveInk` field, or calls `hasSubscribers()`. `onTestRunEnd` is split into two phases in Full mode: a persist program (persistence/classification/baseline/trend, returning `PersistResult`) and a render program that ALWAYS runs — on migration or persist failure it renders `fallbackReports` (built up front, guarded, with empty classifications and no trend) and writes a `persistence failed — results above were rendered but NOT recorded` line to stderr. The render program needs no SQLite (`OutputPipelineLive` + `NodeServices.layer`), then calls `opts.reporter.render(input, kit)` and routes `RenderedOutput[]`. Each project's `test_runs.reason` is derived from that project's own report, not the whole-process Vitest `reason` (`interrupted` stays global). In UI-only mode (`opts.coverageMode === "ui-only"`), the handler short-circuits after `RunFinished` and `filteredModules`: builds reports via `buildAgentReport`, runs a tiny `OutputPipelineLive` + `NodeServices.layer` program to resolve env/executor/format/detail, builds the kit, and routes the renderer output. No `ensureMigrated`, no `DataStore.write*`, no `CoverageAnalyzer.process`, no `HistoryTracker`. **Partial runs (issues #160, #401):** `isPartialRun` decides a run is scoped when Vitest's `filenamePattern` is set, fewer specs started than exist, a `projectFilter` was given, any CLI/programmatic scope filter is set on the stashed Vitest instance's `config.cliOptions` (`--project`, `--tags-filter`, `--changed`, `--related`, `--shard`, via `hasCliScopeFilter`), or the `testNamePattern` snapshot-diff rule (`hasTestNameFilter`) fires (`onInit` snapshots `configOverride.testNamePattern`; at `onTestRunEnd` a changed `current` means partial iff `current` is truthy, an unchanged one means partial iff `cliOptions.testNamePattern` is truthy); a scoped run routes coverage through `CoverageAnalyzer.processScoped`, persists `test_runs.scoped` (+ `scoped_files`), never emits `ThresholdViolation`, and neutralizes the Vitest coverage provider's `thresholds` before its native check runs — the snapshot in `neutralizedThresholdSnapshot` is restored in `onTestRunStart` so watch-mode reruns keep enforcement |
| `services/CoverageAnalyzer.ts` | Effect service tag for coverage processing: `process` for full runs, `processScoped(coverage, opts, testedFiles)` for partial runs (marks the report `scoped`, records `scopedFiles` / `totalFiles`, skips threshold evaluation). Only lives here because the reporter lifecycle class feeds it coverage data; CLI/MCP read pre-processed coverage from SQLite |
| `utils/is-partial-run.ts` | `isPartialRun({ filenamePattern, startedSpecCount, totalSpecCount, projectFilter, cliFilters, testNamePattern })` plus the pure `hasCliScopeFilter(cliFilters)` helper — pure decision functions, five signals, any one makes the run partial. `projectFilter` is the `AgentReporter` constructor option, never the CLI `--project` flag. A CLI `--project` run is NOT caught by the spec-count signal — `globTestSpecifications()` applies the same project filter, so started === total — which is why `cliFilters` exists (issue #401): the stashed Vitest instance's `config.cliOptions`, partial when `project` is non-empty, `tagsFilter` is non-empty, `changed` is truthy, `related` is non-empty, or `shard` is non-empty. `cliOptions` is the raw options object `startVitest(mode, filters, options)` captures, so it carries real CLI flags AND the programmatic filters MCP `run_tests` passes, but NOT settings baked into `vitest.config.ts` — deliberate: a permanent config-file `tagsFilter` is the project's own scope, not a partial run. `testNamePattern` is NOT in `CliScopeFilters`; it is a three-way input `{ cli, initial, current }`: `cli` is `config.cliOptions.testNamePattern` (raw pre-resolution string, `undefined` for a config-file-only pattern), `initial` is the `configOverride.testNamePattern` snapshot `AgentReporter.onInit` takes at startup (Vitest's `_setServer` copies the RESOLVED pattern there — `vitest.config.ts` `test.testNamePattern` merged with `-t` — before reporters exist), `current` is `configOverride.testNamePattern` at `onTestRunEnd`. Rule: if `current` differs from `initial` (compared by RegExp source, not identity) the run is partial iff `current` is truthy (watch-mode `t` filter applied via `Vitest.changeNamePattern` → partial; cleared → full); otherwise partial iff `cli` is truthy (`-t foo` → partial; `-t ""`, unset, or config-file-only → full). Neither source alone works: bare `cliOptions` counts `-t ""` and never sees watch mode; bare `configOverride` would mark every run partial for a project with a config-file pattern |
...
| `utils/discover-strategy.ts` | `DiscoverStrategy` abstract class plus the `DefaultDiscoverStrategy` concrete subclass. Base factory is `DiscoverStrategy.create({ tags, buildProject, classify })`; `.extend({ additionalTags?, buildProject?, classify? })` chains immutable layers. The default strategy ships unit / int / e2e tags and a filename-suffix classifier, and its `buildProject` returns null when neither `src/` nor `__test__/` has tests. Include globs are anchored at the package root (`<path>/src/**/*.{test,spec}.{ts,tsx,js,jsx}` and `<path>/__test__/**/*.{test,spec}.{ts,tsx,js,jsx}`, built from `SRC_DIR` / `TEST_DIR` / `TEST_FILE_GLOB_SUFFIX` imported from `@vitest-agent/sdk`); the exclude list re-states `configDefaults.exclude` plus the root-anchored `<path>/__test__/{fixtures,snapshots,utils}/**` (`TEST_HELPER_DIRS`) — NOT `__test__/**/<dir>/**`, which matched at any depth and silently dropped whole nested suites (issue #251); keep it in step with `classifyTestPath`, which inspects only the segment directly under `__test__/`. The exclude is emitted unconditionally and bounds each `NON_DISCOVERABLE_DIRS` entry (`node_modules`, `.git`, `dist`) under BOTH include roots — `<path>/{src,__test__}/**/<dir>/**`. An anchored include already can't reach a package's own top-level `dist/`; the bounded globs additionally cover build output nested deeper (`src/foo/dist/x.test.ts`), which `configDefaults.exclude` does not, so the emitted config prunes exactly what `findTestFiles` prunes |
| `utils/discover-projects.ts` | `discoverProjects({ strategy?, cwd?, additionalEntries?, fs?, syncOps?, maxDepth? })` runs the unified scan (`fs` defaults to `nodeWalkerFs`, `syncOps` to `@effected/workspaces`'s `nodeSyncOps`; `maxDepth`, when provided, forwards to workspace package enumeration): every workspace package goes through `strategy.buildProject` (null skips), then `.addProject` entries are merged in with name and normalized-path conflict detection. Returns `{ projects: TestProjectInlineConfiguration[] \| undefined; tags }`. Process cache fires only on the strategy-less, additional-entry-less call path, and is invalidated by a cheap per-package signature over exactly `src/` and `__test__/` (`SRC_DIR` / `TEST_DIR` from `@vitest-agent/sdk`; per-directory readdir + mtimeMs, no content reads, no nested-dir walk) so the long-lived MCP server never serves stale include-globs (issue #100). The walks prune `node_modules` / `.git` / `dist` BEFORE recursing — Node's recursive readdir follows symlinks and a fixture `node_modules` would otherwise drag in the pnpm store. Every real scan records an ISO last-scan timestamp into a `Symbol.for("vitest-agent:discovery:last-scan-at")` process-global; `getLastDiscoveryScanTimestamp()` reads it (mcp reads the same slot directly — it cannot import the plugin). A declined package that still looks test-shaped (`isTestShapedPackage`) gets one stderr warning pointing at `vitest-agent agent check-test-path`, deduped module-level so a package warns at most once per process (issue #229) |
| `utils/is-test-shaped-package.ts` | `isTestShapedPackage(pkgPath)` — true when a `__test__/` directory exists at the package root (existence alone, not matching content) or `src/` holds a file matching `TEST_FILE_GLOB_SUFFIX`. Takes the same trailing optional `WalkerFileSystem`; reuses the `@vitest-agent/sdk` constants and the `findTestFiles` walker; drives the one-warning-per-declined-package stderr notice in `discover-projects.ts` |
| `utils/run-script-lock.ts` | File-based advisory lock for `AgentPlugin.runScript`: `resolveRunScriptLockDir()` (`$XDG_DATA_HOME/vitest-agent/runscript-locks/`, same `~/.local/share` fallback as `data.db`, keyed by a truncated SHA-256 of `(cwd, command)`), `acquireRunScriptLock` / `releaseRunScriptLock` / `markRunScriptDone`, plus the four `DEFAULT_*` timings (stale takeover, poll, wait timeout, built-recently window) and `MIN_LOCK_POLL_MS`. Each timing has a `VITEST_AGENT_RUNSCRIPT_*` env override the concurrency e2e suite uses, parsed through the exported `parseLockTimingOverride` (whole-string digits, bounds-checked; garbage/negative/zero falls back to the default). Takeover is two-tier: a lock past `staleMs` is stolen only when the pid recorded in it is dead (`process.kill(pid, 0)` → `ESRCH`) or the owner record is unreadable — a live owner keeps its lock at any age. Release is ownership-gated on a per-acquisition nonce (`RunScriptLock.ownerNonce`), so an owner that was taken over cannot delete the new owner's lock |
...
| `utils/walker-fs.ts` | The filesystem port the four discovery walkers read through: `WalkerFileSystem` (`readDirectory` / `statEntry`, both async), the `WalkerEntry` / `WalkerEntryStat` shapes, and `nodeWalkerFs` — the `node:fs/promises` binding that is the default at every call site, so production behavior is unchanged. Deliberately two operations wide, and deliberately NOT `@effected/workspaces`'s `SyncFileSystem` shape (that port answers entry names; the walkers need the entry type `readdir({ withFileTypes: true })` returns in the same syscall). Tests inject a virtual volume instead of a real tmp tree; see Decision 53 for why the walkers sit on memfs' literal `Volume` view rather than its link-resolving sync port |
...
| `bin/vitest-agent.ts`, `bin/vitest-agent-mcp.ts` | The carrier shims: import `main` from the front end and pass `{ distribution: { name: "@vitest-agent/plugin", version: CURRENT_PLUGIN_VERSION } }` (surfaced by `--version` and MCP `ping`). All process ownership lives in the front ends' `main.ts`. Never add logic here — the cli/mcp packages ship the same bins directly |
| `version.ts` | `CURRENT_PLUGIN_VERSION`, in its own module so the shims read it without importing the plugin graph; `plugin.ts` re-exports it |
| `__test__/workspace-layering.test.ts` | Holds the live package graph to the root `layers.json` through `WorkspaceLayering` (`@effected/workspaces/testing`) — the #412 rank rule (`sdk 1 → ui/sidecar-* 2 → engine/reporter/sidecar 3 → cli/mcp 4 → plugin 5`; root devDepends on plugin only) over runtime fields, no same-layer edge (so cli and mcp never depend on each other), and a DAG across every field. A synthetic upward + `cli -> mcp` edge is the positive control. A new workspace package needs an entry in `layers.json` — a layer, `tooling`, or an `unconstrained` glob, by package name |
| `__test__/bins-packed-install.e2e.test.ts` | `PackedInstall.run` (`@effected/workspaces/testing`) packs the plugin and its runtime closure from `dist/prod/npm/pkg` and installs the carrier tarball into a scratch consumer under npm / pnpm / yarn / bun; asserts `vitest-agent --version` exits 0 with a semver, and under pnpm names the carrier with `@vitest-agent/cli` not linked at top level (the front ends keep their own bins, so the run passes `allowSharedBins: true`; npm and bun may link the hoisted cli bin instead — same program, no carrier suffix), and that `vitest-agent-mcp` passes `McpProbe.initialize` (`@effected/mcp/testing`; empty stderr, exit 0). The carrier's OWN bins are also run under every manager via `consumer.runCarrierBin` / `carrierCommand` (resolved through the carrier's `bin` map), so the shim path and its carrier suffix are proven even where a front end holds the `.bin` slot. The closure is planned up front with `PackedInstall.closure` (same options as `run`, `workspaceOverrides: true` so a dogfood loop's `file:` overrides are packed too) to size the timeout. Every manager required under CI, missing ones skipped locally. Needs `pnpm run build` (prod) and network; skips when prod dist is absent or on Windows |
...
Internally calls `discoverProjects({ strategy?, cwd?, maxDepth?,
...
`strategy`, `cwd`, and `maxDepth`. With no argument, the builder uses
...
recovers via the stale-takeover path (dead-pid probe, then age). Tune
the four timings in tests through the `VITEST_AGENT_RUNSCRIPT_*` env
overrides, never by lowering the production defaults. Serialization is
best-effort by contract: a waiter still blocked after
`waitTimeoutMs` runs the command anyway, unserialized, instead of
hanging the caller's test run.
...
- Changing project discovery: edit `utils/discover-strategy.ts` for
  ...
  `snapshots/`) are excluded automatically only when they sit DIRECTLY
  under `__test__/`; nested ones (`__test__/unit/utils/`) are ordinary
  suites and must stay discoverable (issue #251). A null
...
- Adding or editing a filesystem walk: read through the
  `WalkerFileSystem` port from `utils/walker-fs.ts`, taken as a trailing
  optional param defaulting to `nodeWalkerFs` — never import `node:fs`
  into a walker. That keeps the walks testable against a virtual volume.
...
- [`../../okf/decisions/59-detect-partial-runs-and-neutralise-the-native-threshold-check.md`](../../okf/decisions/59-detect-partial-runs-and-neutralise-the-native-threshold-check.md)
  Load when touching `isPartialRun` / `hasCliScopeFilter`, the
  `cliFilters` and `testNamePattern` signals (issue #401), or the
  coverage-threshold neutralise-and-restore pair around a scoped run.
