...
| `utils/is-partial-run.ts` | `isPartialRun({ filenamePattern, startedSpecCount, totalSpecCount, projectFilter })` — pure decision function. `projectFilter` is the `AgentReporter` constructor option, never the CLI `--project` flag; a `--project` run is caught by the spec-count signal instead |
