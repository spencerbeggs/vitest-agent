...
| `__test__/bins-packed-install.e2e.test.ts` | `PackedInstall.run` (`@effected/workspaces/testing`) packs the plugin and its runtime closure from `dist/prod/npm/pkg` and installs the carrier tarball into a scratch consumer under npm / pnpm / yarn / bun; asserts `vitest-agent --version` exits 0 with a semver, and under pnpm names the carrier with `@vitest-agent/cli` not linked at top level (npm/yarn/bun may link the hoisted cli bin instead — same program), and that `vitest-agent-mcp` passes `McpProbe.initialize` (`@effected/mcp/testing`; empty stderr, exit 0). Every manager required under CI, missing ones skipped locally. Needs `pnpm run build` (prod) and network; skips when prod dist is absent or on Windows |
