/**
 * tsdown SEA build config for `vitest-agent-sidecar-darwin-x64`.
 *
 * This child package compiles ITS OWN platform binary — a Node Single
 * Executable Application (SEA) for `darwin/x64` only. The five
...
// `darwin`; the published `os` value is `darwin`.
