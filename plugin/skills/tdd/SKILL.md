...
2. **Explicit task-id escape hatch (manual, last resort).** Set `VITEST_AGENT_TDD_TASK_ID=<tddTaskId>` in the subagent's environment. `plugin/hooks/post-tool-use/tdd-artifact.sh` forwards it as `--tdd-task-id` on every `record tdd-artifact` call, which bypasses `chat_id` → session → task resolution (and therefore the conversation-tree fallback too) and writes straight to that task's current open phase. Use this only when you already know the fallback above isn't resolving correctly for the session shape you're operating under — it is a deliberate override, not a default. If a `tdd_phase_transition_request` denial's `remediation.humanHint` mentions artifacts landing under "a different session of this conversation", that message is telling you to set this variable (or reconsider whether an unnamed subagent dispatch was possible after all).
