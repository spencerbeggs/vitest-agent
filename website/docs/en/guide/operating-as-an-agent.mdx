...
7. **Judge a run by its collected count, not by `0 failed`.** The totals line
...
8. **A rendered result is not always a recorded one.** If stderr carries
...
9. **Some "leaks" are guardrail tests** that assert on their own output — do not
   silence them.
