...
Channel-event handling (when Claude Code's channels are active) and the task-list rendering rules live in the `tdd` skill. If you are the main agent and channels are active, refer to `plugins/claude-code/skills/tdd/SKILL.md` for the event handler table and the flat `[G<n>.B<m>]` label-encoding convention.
