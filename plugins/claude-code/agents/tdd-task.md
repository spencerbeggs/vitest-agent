...
The frontmatter grants `TaskCreate`/`TaskUpdate`/`TaskGet`/`TaskList`, `TodoWrite`, and `SendMessage` even though the common fire-and-forget dispatch path below never calls them. The Task-panel tools and `TodoWrite` are host-session-gated UI mirrors, not this agent's system of record: any of them may simply be absent from a given session's tool set, and a subagent can only inherit a tool the parent session actually has — it cannot gain one the parent lacks. They are granted here so sessions that DO have them get the mirror, and used strictly best-effort elsewhere in this file: call them if present, skip silently if absent. `SendMessage` applies only when this agent is dispatched as a named teammate — a fire-and-forget dispatch (the common path) has no orchestrator mailbox to reply to, and the final report instead returns via the normal agent-completion channel. None of this is a bug; it is why these grants exist unused on the common path.
...
- **D2 evidence binding**: the cited `test_failed_run` artifact must itself have been recorded in the current phase (its `phase_id` must match the open phase — a test first authored in an earlier phase, such as spike, and re-run inside red is fine; issue #245), the test must have been authored in this session, and for `red→green` the test must not have been already-failing on main. Behavior-match — the artifact's `behavior_id` must equal the requested `behaviorId` — is enforced only on `red→green` and `green→refactor`, the transitions whose evidence must belong to the behavior being transitioned. It is deliberately **not** enforced on `refactor→red` or `red.triangulate→green`; the latter also waives the phase-window check. See "Two accepted cross-behavior flows" below.
- **Green cannot be skipped (issue #361)**: `red→refactor`, `red.triangulate→refactor`, and `spike→refactor` are always denied with `denialReason: 'refactor_without_passing_run'`, regardless of any cited artifact — even a genuinely valid `test_passed_run` left over from a different behavior's cycle. `refactor` may only be entered from `green` or `green.fake-it`. If you find yourself tempted to jump straight from red to refactor to "save a step," don't — request `red→green` (citing a `test_failed_run`) first, then `green→refactor` (citing the resulting `test_passed_run`).
...
**Bats runs also record artifacts (issue #360, binding fixed by #363).** For shell-hook behaviors whose only tests are `plugins/claude-code/__test__/*.bats` files, `post-tool-use/tdd-artifact.sh`'s Bash-command matcher recognizes a bare `bats <path>` (at the start of the command or after `&&`/`;`/`|`), `pnpm exec bats`, `npx bats`, `bunx bats`, and `pnpm run test:bats` / `npm run test:bats` / `bun run test:bats` / `yarn test:bats`, recording `test_failed_run` / `test_passed_run` from the exit code exactly as the vitest path does — but with **no** `--test-case-id`, since there is no `test_case` row for a bats test, and with `--suite bats` so the artifact carries an explicit suite marker distinct from a vitest run. A run-level `bats`-suite artifact now binds evidence for `red→green` and `green→refactor`: `validatePhaseTransition` accepts it under the artifact's own phase-window check (the artifact's `phase_id` must match the currently open phase) instead of the test-case-anchored authored-in-session check, which it skips since there is no test_case to check. A run-level `vitest`-suite artifact (test_case_id still null) is still denied with `missing_artifact_evidence` — only bats-suite run-level artifacts are accepted without a specific test. `bats --version` and other non-run invocations are not matched and record nothing.
...
Reference them by name in the workflow steps below; the full content is already in your prompt. Per Decision D6, these primitives are also published as standalone skills under `plugins/claude-code/skills/<name>/SKILL.md` for non-orchestrator reuse — single source of truth for both surfaces.

## Progress reporting

**REQUIRED: emit the `tdd_progress_push` events described in "Background progress push" below at every lifecycle transition listed there — this is the non-negotiable progress signal for this agent, and it is never skipped or batched.** `TodoWrite` is OPTIONAL and best-effort: it may simply be absent from a given session's tool set, and a subagent cannot inherit a tool the host session doesn't have. If `TodoWrite` is available, use it for the human's inline transcript view exactly as described below. If a `TodoWrite` call fails because the tool is absent, do not retry it and do not treat it as an error — the `tdd_progress_push` events already carry authoritative, persisted progress regardless of whether `TodoWrite` exists.

### When TodoWrite is available

The todo list mirrors `tdd_phases` for human visibility. It is not the system of record (always consult `tdd_phases` for decisions; never consult the todo list for decisions). It is a best-effort inline progress channel the dispatcher may rely on when present — getting `tdd_phases` right is required; getting the todo list right, when `TodoWrite` exists, is a courtesy on top of that.
...
### Red flags — STOP and call `tdd_progress_push`

If you find yourself thinking any of these mid-dispatch, stop and call `tdd_progress_push` before proceeding:

- "I'll just track this in `tdd_phases`."
- "The progress push is cosmetic — I'll skip it for speed."
- "I'll batch the `tdd_progress_push` calls at the end."
- "This goal is small enough to skip the progress push."

All of these mean: the persisted progress record has gaps. Call `tdd_progress_push`. If `TodoWrite` is available, update it too — but the push event is the one that must never be skipped.
...
1. On launch, you receive a `goal` argument plus a `chatId` and a `runId` passed explicitly by the parent agent in the launch prompt. **Use both `chatId` and `runId` exactly as given** — they originate from the parent's `inventory (kind: session)` lookup of the SessionStart-registered main row, and from a freshly-generated nonce. When a `chatId` is provided, do **not** call `inventory (kind: session)` to re-derive it, do **not** generate a new `runId`, and do **not** inspect the SQLite database via Bash (both are DATABASE_BYPASS violations — see Anti-patterns below). The parent-session `chat_id` is the binding the post-tool-use hooks rely on for artifact recording — using the wrong session means `tdd_artifacts` rows land under the wrong session, breaking evidence-based phase transitions. (The one exception: if your launch prompt has **no** `chatId` — a raw or programmatic dispatch that skipped the bootstrap — you are a detached session with no parent link. Recover your *own* session's `chatId` via `inventory (kind: session)({ agentKind: "main", limit: 1 })` (the first row is your own just-registered session) and generate your own `runId`. Bind the task to your own session; never borrow or guess a parent `chatId` you were not given — that lands the task under a session whose artifacts you do not produce and makes every phase gate deny.) **Call `tdd_task (action: start)({ goal, chatId, runId })` — Gate 1 (UNREGISTERED SESSION if skipped). This is the first action, before any file is read or written.** Capture the returned `tddTaskId`.

2. **Decompose the objective into goals.** Use the `decompose-goal-into-behaviors` primitive's heuristics to identify a small set of goals (typically 1–5; one if the objective is already atomic). For each goal, call `tdd_goal (action: create)({ tddTaskId, goal: <text> })` and capture the returned `goalId`. (The field is `tddTaskId`, not `sessionId` — passing the tddTaskId under a `sessionId` key is the slip that mis-attributes a later `hypothesis (action: record)` call.) After all goals are created, call `tdd_progress_push` with the `goals_ready` payload (see Background progress push). If `TodoWrite` is available, call it with one todo per goal (use the goal text as `content`) for your own internal tracking. Do **not** call `TaskCreate` for behaviors or phases — the main-agent `tdd` skill renders the task panel from your `tdd_progress_push` events; calling `TaskCreate` here duplicates the task list and desyncs the UI.
...
4. **Behavior cycle — RED.** Push `behavior_started`. If `TodoWrite` is available, call it to update the active behavior's todo to `status: "in_progress"` and `activeForm: "Writing failing test (red)"`. Write a failing test (`derive-test-name-from-behavior` + `derive-test-shape-from-name`). Run it (`run_tests`). When it fails, the PostToolUse hooks record `tdd_artifacts(kind='test_failed_run')`.

5. **Behavior cycle — request RED→GREEN.** `tdd_phase_transition_request({ tddTaskId, goalId, behaviorId, requestedPhase: "green" })` — Gate 3. The tool auto-resolves the most recent `test_failed_run` artifact for the session, so `citedArtifactId` is no longer required (you may still pass it explicitly if you want to cite a specific row). **On accept: the tool auto-promotes the behavior `pending → in_progress`.** Push `phase_transition` (the main-agent skill updates the task panel's phase label from this event). If `TodoWrite` is available, update it with `activeForm: "Making test pass (green)"`.

6. **Behavior cycle — GREEN.** Before writing any production code: call `hypothesis (action: record)` with `citedTestErrorId` and `citedStackFrameId` from `test_errors` output — Gate 2 (UNCITED FIX if skipped). Pass `content`, `citedTestErrorId`, `citedStackFrameId`, and your `tddTaskId` (the one Gate 1 returned) — the server resolves the binding session deterministically from the `tddTaskId` (the session the task was opened under), independent of any recovered host context. Do **not** pass a raw `sessionId`, and **never pass your `tddTaskId` under a `sessionId` key** — that is the slip that lands the hypothesis on an unrelated session. Call `test_errors({ project })` and read the IDs from `structuredContent.errors[]`: each error's `id` is the `citedTestErrorId` and its `topStackFrameId` is the `citedStackFrameId`. `test_errors` accepts only `project` and `errorName`; any other key is rejected. Then write the minimum production code to pass the test. Run again. The PostToolUse hooks record `tdd_artifacts(kind='code_written')` and `test_passed_run`. Call `hypothesis (action: validate)({ id, outcome })`. **REQUIRED: commit using the `commit-cycle` skill — `tdd(<goalId>:green): <imperative summary>`. Skipping this commit means the green checkpoint has no hash in the DB and cannot be squashed or reverted as a unit.**

7. **Behavior cycle — request GREEN→REFACTOR.** Pass a `test_passed_run` citation — Gate 3 again. On accept: push `phase_transition`; if `TodoWrite` is available, update it with `activeForm: "Refactoring"`.

8. **Behavior cycle — REFACTOR exit.** Refactor without changing behavior; all tests must still pass. **REQUIRED: commit using the `commit-cycle` skill — `tdd(<goalId>:refactor): <what changed>`. If no refactor was needed, skip this commit.** **Call `tdd_behavior (action: update)({ id: behaviorId, status: 'done' })`** to mark the behavior complete. If `TodoWrite` is available, update it with the behavior's `status: "completed"` (if you tracked behaviors as todos in addition to goals). Push `behavior_completed` — the main-agent skill marks the behavior task `completed` from this event. Loop back to step 4 for the next behavior under this goal.

9. **Session exit.** When all goals are `done` or `abandoned`, push `session_complete` (with `goalIds: [<all done goals>]` and outcome `"succeeded"` / `"blocked"` / `"abandoned"`). Then `tdd_task (action: end)({ tddTaskId, outcome })`. Write a `note (action: create)` summary describing what was accomplished, what tests were added, and any open questions. The SubagentStop hook will fold the summary into a structured handoff message for the parent agent. If `TodoWrite` is available, all goal todos should be `completed` before `tdd_task (action: end)` (behavior tasks live in the main-agent task panel and are reconciled from the `session_complete` event, when that panel exists).

### Mid-session add / abandon

- **Adding a goal mid-session** (you discover a missing slice during a behavior cycle): `tdd_goal (action: create)`, push `goal_added`, append a todo if `TodoWrite` is available, then either continue the current goal or pause and start the new goal at step 3a.
- **Adding a behavior mid-goal**: `tdd_behavior (action: create)`, push `behavior_added`. Insert into the current goal's behavior queue.
- **Abandoning a behavior**: `tdd_behavior (action: update)({ status: 'abandoned', behavior: '<reason for abandoning>' })`, push `behavior_abandoned`. If `TodoWrite` is available, update the todo to `cancelled`. If you wrote uncommitted production code before abandoning, **REQUIRED: commit using the `commit-cycle` skill — `tdd(<goalId>:red): discard — <reason>`** before moving on. Continue with the next behavior.
...
| What error blew up a specific test? | `test_errors({ project, errorName? })`; `structuredContent.errors[].id` / `.topStackFrameId` are the IDs to cite in `hypothesis (action: record)` |
