...
_turn_out=$(cd "$cwd" && $pm_exec vitest-agent agent record turn \
	...
	_rc=$?
	hook_error "$_HOOK" "record turn tool_result rc=$_rc cc=$chat_id tool=$tool_name: $(cat "$_turn_err")"
}
rm -f "$_turn_err"
...
case "$tool_name" in
	Edit|Write|MultiEdit)
		...
		_edit_out=$(cd "$cwd" && $pm_exec vitest-agent agent record turn \
			...
			_rc=$?
			hook_error "$_HOOK" "record turn file_edit rc=$_rc cc=$chat_id file=$file_path: $(cat "$_edit_err")"
		}
		rm -f "$_edit_err"
