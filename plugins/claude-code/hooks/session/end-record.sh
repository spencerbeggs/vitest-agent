...
case "$reason" in
	...
	*)
		...
		# wait resolves as soon as the shim exits. `3>&-` closes the
		# fenced stdout descriptor hook-output.sh parked on fd 3; leaving
		# it open would hand the worker a handle on the host's real stdout
		# pipe and defeat the whole detach.
		log_dir="${HOME}/.claude/session-env/${chat_id}"
		mkdir -p "$log_dir" 2>/dev/null || true
		nohup bash "$worker" "$chat_id" "$cwd" "$reason" quiet \
			</dev/null >>"${log_dir}/session-end-worker.log" 2>&1 3>&- &
