...
This directory is a **file-based Claude Code plugin**. It IS a pnpm workspace member (`pnpm-workspace.yaml` includes `plugins/*`) but is **not** published to npm. It ships alongside the npm packages through the Claude marketplace.
...
- **Versioned through the `@vitest-agent/claude-code-plugin` tracking package** — `plugins/claude-code/package.json` is `"private": true` with no `publishConfig`; its only job is to give changesets something to version. `.changeset/config.json` maps its `versionFiles` to `.claude-plugin/plugin.json` at `$.version`, so CI bumps the `package.json` version and the manifest version together, then cuts a git tag and GitHub Release with no npm publish. Write plugin changesets against `@vitest-agent/claude-code-plugin`; a changeset for any other package does nothing here. (The former runtime version-drift stderr warning was removed; see the root CLAUDE.md "Cross-package versioning" section.)
...
```text
plugins/claude-code/
├── package.json         # @vitest-agent/claude-code-plugin tracking package (private, unpublished)
├── .claude-plugin/
│   └── plugin.json      # CC plugin manifest: mcpServers, hooks, skills, agents, commands
├── __test__/            # BATS tests for hook scripts
│                        #   hook-output-project-dir.bats, hook-stdout-fence.bats,
│                        #   sidecar-prefilter.bats, sidecar-layer2.bats,
...
Every hook that shells out to the CLI resolves it through `detect_vitest_agent_bin` (`hooks/lib/detect-pm.sh`): `VITEST_AGENT_CLI_CMD` if set, else the project's own `node_modules/.bin/vitest-agent` (linked by the `@vitest-agent/plugin` carrier — issue #412; returned as a RELATIVE path because call sites expand it unquoted after `cd "$cwd"`, so a space in the project path cannot word-split it), else `vitest-agent` on `PATH`, else fail — the function returns non-zero with no output and every call site must fall back to its own no-op emission and exit 0 rather than block. Hooks never dispatch through a package manager and never fall back to `npx` (a hook fires far more often than a server starts). `bin/start-mcp.sh` is a different loader with a different contract: it applies the same local-bin preference to `node_modules/.bin/vitest-agent-mcp` before falling back to a major-pinned `npx --yes @vitest-agent/mcp@5`. Covered by `__test__/bin-preference.bats`.
...
| `post-tool-use/test-run.sh` | `PostToolUse` (Bash) | Detects vitest invocations in the Bash command; records the run trigger and test-case turns via two `vitest-agent agent record` calls (both `>/dev/null 2>&1` — `record test-case-turns` prints a JSON object that otherwise leaks into the payload); on non-zero exit code (read from `.tool_response.exit_code`) injects `<test_failure_guidance>` pointing the agent at MCP tools |
| `post-tool-use/git-commit.sh` | `PostToolUse` (Bash) | Detects `git commit` invocations; records commit metadata into `commits` table |
| `post-tool-use/tdd-artifact.sh` | `PostToolUse` (Write/Edit/Bash/run_tests, tdd-task) | Records `test_written`, `test_failed_run`, `test_passed_run`, `code_written` artifacts into `tdd_artifacts`. Bash test-runner detection covers vitest/jest, PM `test` scripts, and bats (`bats <path>`, `pnpm exec bats`, `npx bats`, `bunx bats`; `bats --version` is excluded — issue #360). When `VITEST_AGENT_TDD_TASK_ID` is set it forwards `--tdd-task-id` so the CLI skips session resolution. Covered by `__test__/tdd-artifact-bats.bats` |
...
`hook-output.sh` (`hooks/lib/`) centralizes every JSON shape a hook may emit: `emit_noop`, `emit_allow`, `emit_deny`, `emit_additional_context`, `emit_system_message`, plus `emit_raw` — the escape hatch for PreToolUse `updatedInput`, whose object is tool-specific and cannot be centralized as a fixed shape. `emit_raw` only routes bytes (`cat >&3`); the caller still owes the `jq -n --arg` encoding and ends its pipeline with `| emit_raw` (`pre-tool-use/bash.sh`, `pre-tool-use/mcp-run-tests.sh`). All user strings flow through `jq -n --arg` to prevent embedded quotes or newlines from breaking the payload. It also sets `VITEST_AGENT_PROJECT_DIR` from `CLAUDE_PROJECT_DIR` at source time (only when not already set and then exports it), anchoring every `vitest-agent` CLI call in a hook to the same project root the MCP server uses. This is load-bearing for subagent TDD recording: without it, a hook running from a monorepo sub-package `cwd` would resolve a different `data.db` and silently split evidence across two databases. The BATS test `__test__/hook-output-project-dir.bats` covers the project-dir propagation contract.

At source time it also installs a **stdout fence**: `exec 3>&1 1>&2` behind the non-exported `_VITEST_AGENT_HOOK_STDOUT_FENCED` guard, with every emitter writing to `>&3`. Claude Code parses hook stdout as one JSON object, so any stray byte on fd 1 — from the hook itself, or from a spawned `vitest-agent` CLI whose call site forgot `>/dev/null` — corrupts the payload (issue #373). The fence diverts that noise to stderr instead of trusting every call site. Command substitution is unaffected (`$(cmd)` installs its own pipe on fd 1). Anything that detaches a background worker MUST close the inherited descriptor with `3>&-` — see `session/end-record.sh`. The guard is deliberately not exported so a nested script fences its own fd 1 rather than inheriting the parent's fd 3. Covered by `__test__/hook-stdout-fence.bats`, which also carries a structural invariant test: for every hook sourcing `hook-output.sh`, the count of statement-level `^jq -n` lines must equal the count of `| emit_raw`, so a future hand-rolled payload that skips the helper fails the suite instead of silently emitting nothing.
...
| --- | --- | --- |
| `/setup` | `commands/setup.md` | Verify Vitest 5.0+ and peer dependencies, then emit the canonical 2.0 `vitest.config.ts` |
