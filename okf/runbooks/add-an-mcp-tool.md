...
2. **If the tool is action-keyed** (a CRUD family collapsing into one
   ...
   and the handler's branch coverage cannot drift from each other.
3. **Add the tool to `Kit` and its handler to `toolHandlers` in
   ...
   surprise.[^toolkit] Every tool registers through
   `registerStrictToolkit`, never `McpServer.toolkit` directly — that is
   what serves `additionalProperties: false` at every object level and
   rejects an unknown key instead of silently widening the
   query.[^register-toolkit]
...
5. **Write a harness test.** `__test__/utils/harness.ts` builds the real
   `ServerLayer` over `Stdio.layerTest` queues — no child process — so a
...
[^toolkit]: `../../packages/mcp/src/toolkit.ts:38-107`
[^ping-tool]: `../../packages/mcp/src/tools/ping.ts:1-54`
[^register-toolkit]: `../../packages/mcp/src/register-toolkit.ts:1-27`
[^idempotency]: `../../packages/mcp/src/idempotency.ts:37`
[^harness]: `../../packages/mcp/__test__/utils/harness.ts:1-11`
