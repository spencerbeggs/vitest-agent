...
The `**/dist/**` exclude that the widened glob required is also gone — an anchored include cannot reach a package's own top-level `dist/`, so nothing needs to exclude it. The walk that decides which projects to emit still prunes `dist/` (along with `node_modules/` and `.git/`) wherever it appears, from the shared `NON_DISCOVERABLE_DIRS` constant in `@vitest-agent/sdk`.
